...
function syncSelectionWithCurrentFiles() {
  const existingPaths = new Set(state.files.map((item) => item.path));
...
function renderListRows() {
  return state.files
    .map((item) => {
      ...
      const checked = isSelectedPath(item.path) ? "checked" : "";
      const selectDisabledAttr = "";
      ...
      actions.push(
        `<button class="btn btn-primary" data-download="${encodedPath}" data-entry-type="${item.type}" data-filename="${encodedName}">下载</button>`
      );

      if (item.type === "file" && state.isAdmin && item.storageType === "temporary") {
        actions.push(
...
async function collectFilesRecursively(rootPath) {
  ...
  async function walk(currentPath) {
    const payload = await requestJson(
      `${baseUrl()}/api/files?path=${encodeURIComponent(currentPath || "")}`
    );
...
async function downloadFolderContents(relativePath, folderName) {
  try {
    const files = await collectFilesRecursively(relativePath);
    if (!files.length) {
      showToast("文件夹为空，无可下载文件", "error");
      return false;
    }
    ...
    for (const file of files) {
      // 逐个按原始文件方式下载
      const ok = await downloadFile(file.path, file.name, "file");
      if (ok) {
        success += 1;
      }
    }

    const label = folderName || relativePath || "该文件夹";
    showToast(
      `${label} 下载完成：成功 ${success}/${files.length}`,
      success ? "success" : "error"
    );
    return success > 0;
  } catch (error) {
    showToast(`文件夹下载失败：${error.message}`, "error");
    return false;
  }
}

async function downloadFile(relativePath, fileName, entryType = "file") {
  // 文件夹：改为递归列出其中的所有文件，逐个按“原有单文件下载逻辑”下载
  if (entryType === "folder") {
    return downloadFolderContents(relativePath, fileName);
  }
...
async function downloadSelectedEntries() {
  const selected = state.files.filter((item) => state.selectedPaths.has(item.path));
  if (!selected.length) {
    showToast("请先选择要下载的文件或文件夹", "error");
    return;
  }

  let success = 0;
  for (const item of selected) {
    const ok = await downloadFile(item.path, item.name, item.type);
    if (ok) {
      success += 1;
    }
  }
